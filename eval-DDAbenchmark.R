...
list_dset <- list(
    c("dda.choi.mq", "dda.choi.pd", "dda.choi.pg", "dda.choi.sl"), 
    c("dda.iprg.mq", "dda.iprg.pg", "dda.iprg.sl")
)
...
nb_ftr_all <- bind_rows(nb_ftr_all) %>% 
    ...
    mutate(expt = expt %>% str_replace_all(c("Dda" = "DDA", "Iprg" = "iPRG"))) %>%
    mutate(tool = tool %>% str_replace_all(
        c("mq" = "MaxQuant", "pd" = "Proteome Discoverer", "pg" = "Progenesis", "sl" = "Skyline")
    ))

# Number of features before/after feature selection (Fig. 5a)
nb_ftr_all %>% 
...
# ggsave("boxnb-dda.png", width = 12, height = 4)

# Load all group comparison results ---------------------------------------

meth <- c("All", "Proposed", "Top5", "Top3")
meth_code <- c("all", "inf", "top5", "top3")
...
plot_ftrpair <- function(one_processed, one_runsum, text_title) {
    ...
    pair_processed %>% 
        ...
        annotate("text", x = 2, y = 34, label = "Condition1", size = 6) + 
        annotate("text", x = 5, y = 34, label = "Condition2", size = 6) + 
        annotate("text", x = 8, y = 34, label = "Condition3", size = 6) + 
        annotate("text", x = 11, y = 34, label = "Condition4", size = 6) + 
...
# Example of Progenesis (Fig. 5b)
dset_name <- "dda.iprg.pg"; dset_text <- "DDA iPRG (Progenesis)"
...
test_res_all %>% 
    filter(Protein == prot_name, Label == "C2-C1") %>% 
...
df_dummy <- data.frame(
    ...
    expt = rep(c("DDA Choi", "DDA iPRG"), each = 4)
)
test_res_all %>% 
    filter(is_BG) %>% 
    filter(!is.na(pvalue)) %>% 
    mutate(method = factor(method, levels = colmeth)) %>% 
    mutate(tool = ifelse(tool=="Proteome Discoverer", "Proteome\nDiscoverer", tool)) %>% 
    ggplot(aes(tool, abs(log2FC - trueLog2FC))) + 
    geom_boxplot(aes(fill = method)) + 
    geom_text(aes(label = method), nudge_x = c(-0.27, -0.09, 0.09, 0.27) - 0.03, 
...
test_res_all %>% 
    ...
    theme(strip.background = element_rect(fill = "white"), strip.text = element_text(face="bold")) + 
    theme(legend.position = "bottom")
# ggsave("boxse-10-dda_bg.png", width = 10, height = 4.5)
...
sp_curves_all %>% 
    ...
    geom_line() + 
    scale_linetype_manual(values = c(rep("solid", 2), "dashed", "dotted")) +
    scale_size_manual(values = c(rep(0.55, 2), 0.5, 0.5)) +
    scale_color_manual(values = myPalette) + 
    coord_cartesian(xlim = c(0, 0.15), ylim = c(0.55, 1)) + 
    facet_wrap(~ tool) + 
    labs(x = "False positive rate", y = "Sensitivity", color = "Method") + 
    theme_bw() + 
    theme(strip.background = element_rect(fill = "white"), strip.text = element_text(face="bold")) + 
    theme(legend.position = "bottom")
...
sp_curves_all %>% 
    ...
    theme(strip.background = element_rect(fill = "white"), strip.text = element_text(face="bold")) + 
    theme(legend.position = "bottom")
# ggsave("rocs-10-dda_iprg_doc.png", width = 6, height = 3)
...
# Euler diagram for the Choi benchmark (Fig. S11)
bm <- "DDA Choi"
...
# Euler diagram for the iPRG benchmark (Fig. S11)
bm <- "DDA iPRG"
for (j in seq_along(meth)) {
    test_tmp <- test_res_tp %>% 
        filter(expt == bm) %>% 
...
est_nbftr %>% 
    ...
    labs(x = "Method", y = "Median absolute estimation error", 
         color = "Benchmark", shape = "Tool", title = "DDA benchmarks") + 
    theme_bw(base_size = 12) + 
    theme(strip.background = element_rect(fill = "white"), strip.text = element_text(face="bold")) + 
    theme(legend.position = "bottom")
# ggsave("lineerr-ftr-dda.png", width = 9, height = 4)
